...
// ✅ Renamed onChange → onToggle to avoid clash with native HTML onChange
        ...
        {open && (
            <div className="filter-body">
              {section.options.map(opt => {
                const enumVal = LABEL_TO_ENUM[opt] ?? opt;
                return (
                    <label key={opt} className="filter-option">
                      <input
                          type="checkbox"
                          checked={selected.includes(enumVal)}
                          onChange={() => onToggle(section.key, enumVal)} // ✅ onToggle, not onChange
                      />
      ...
      <aside className="sidebar" aria-label="Artwork filters">
        ...
        {FILTER_SECTIONS.map(section => (
            <FilterSection
                ...
                onToggle={onFilterChange} // ✅ pass as onToggle
            />
        ))}
      </aside>
  );
